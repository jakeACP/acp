...
- **Political Parties Module**: Full parties directory at `/parties` with filtering/sorting by transparency, ballot access, and political compass position. Individual party profile pages at `/parties/:partyId` showing compass chart, policy positions, ballot access map, endorsements, controversies, and community ratings. Admin management panel at `/admin/parties` (CRUD + seed). Candidate profile "Endorsed By" integration. 15 seeded parties. All parties presented neutrally.
