          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="relative">
              ...
              {mentionQuery !== null && (mentionResults.length > 0 || mentionLoading) && (
                <div className="absolute left-0 top-full z-50 mt-1 w-80 max-h-64 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg">
                  {mentionLoading && <div className="px-4 py-3 text-sm text-slate-500">Searching politicians...</div>}
                  {mentionResults.map((pol: any) => (
                    <button
                      key={pol.id}
                      type="button"
                      className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-left"
                      onClick={() => {
                        if (!pol.handle) return;
                        const cursor = textareaRef.current?.selectionStart ?? content.length;
                        const before = content.slice(0, cursor);
                        const after = content.slice(cursor);
                        const replaced = before.replace(/@\w*$/, `@${pol.handle}`);
                        const newContent = replaced + after;
                        setContent(newContent);
                        const tag = `@${pol.handle}`;
                    ...
                    >
                      {pol.photoUrl ? (
                        <img src={pol.photoUrl} alt={pol.fullName} className={`w-8 h-8 rounded-full object-cover shrink-0 ${pol.isVerified ? "" : "grayscale"}`} />
                      ) : (
                        <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center shrink-0">
                          <span className="text-xs font-bold text-slate-500">{pol.fullName?.charAt(0)}</span>
                        </div>
                      )}
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{pol.fullName}</div>
                        <div className="text-xs text-blue-600 dark:text-blue-400">@{pol.handle}</div>
                      </div>
                    </button>
                  ))}
                </div>
