...
export function UserSidebar() {
  ...
  const { data: friendData, isLoading: isFriendCountLoading } = useQuery<{ friendCount: number }>({
      ...
      <Card className="floating-card bg-card border border-border dark:border-border">
        <CardContent className="p-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 text-center">
            ...
            >
              <p className="text-xl font-bold text-foreground">{isFriendCountLoading ? "—" : (friendData?.friendCount ?? 0)}</p>
