...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  app.post("/api/admin/sigs/seed-xlsx", ensureStateAdmin, async (req, res) => {
    const SIG_SEED_DATA = [
      ...
      { name: "1199 SEIU NEW YORK STATE POLITICAL ACTION FUND", tag: "1199_SEIU_NY_PAF", description: "Local 1199 SEIU represents healthcare workers in New York State. Major force in NYC politics, advocating for workers' rights and healthcare access.", category: "Labor Union", sentiment: "positive", dataSourceName: "1199 SEIU New York State", dataSourceUrl: "https://1199seiu.org", disclosureNotes: "" },
...
Only include people you are confident about. Return empty arrays/null if unknown. Return ONLY valid JSON.`
  ...
  app.post("/api/admin/agent-keys", ensureAdminOnly, async (req: AgentAdminRequest, res) => {
    ...
    try {
      ...
      res.status(201).json({ key: serializeAgentKey(key), rawKey });
    } catch {
