...
export interface IStorage {
  ...
  createAcePledgeRequest(data: InsertAcePledgeRequest): Promise<AcePledgeRequest>;
  getAcePledgesByPolitician(politicianId: string): Promise<AcePledgeRequest[]>;
  getAllAcePledgeRequests(status?: string): Promise<(AcePledgeRequest & { sigName?: string; politicianName?: string })[]>;
  reviewAcePledgeRequest(id: string, status: string, reviewedBy: number, reviewNote?: string): Promise<AcePledgeRequest>;
  ...
  // API Keys
  createApiKey(data: { userId: string; name: string; keyHash: string; keyPrefix: string }): Promise<ApiKey>;
  ...
  getAgentApiKeyById(id: string): Promise<AgentApiKey | undefined>;
  findAgentApiKeyByHash(keyHash: string): Promise<AgentApiKey | undefined>;
  updateAgentApiKey(id: string, data: Partial<InsertAgentApiKey>): Promise<AgentApiKey>;
  ...
  // Pledge Requests
  submitPledgeRequest(userId: string, pledgeId: string, statement: string): Promise<any>;
  getUserPledges(userId: string): Promise<any[]>;
  getApprovedPledges(userId: string): Promise<any[]>;
  getPledgeRequests(status?: string): Promise<any[]>;
  reviewPledgeRequest(id: string, status: string, reviewedBy: string, reviewNote: string): Promise<void>;
  ...
  queueZipCandidateImports(zipCode: string, candidates: Array<{ name: string; office: string; raceLevel: string; party?: string; city?: string; state?: string; district?: string; matchStatus?: "in_db" | "possible_duplicate" | "new" }>, source?: string): Promise<{ queued: number; duplicates: number; total: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByPhone(phone: string): Promise<User | undefined> {
    const [user] = await db.select().from(users).where(eq(users.phoneNumber, phone.trim()));
    return user || undefined;
  }
  ...
  async updateSignal(id: string, data: { title?: string; description?: string; tags?: string[]; thumbnailUrl?: string }): Promise<Signal> {
    const [signal] = await db
      .update(signals)
      .set(data)
  ...
  async createAcePledgeRequest(data: InsertAcePledgeRequest): Promise<AcePledgeRequest> {
    const [result] = await db.insert(acePledgeRequests).values(data).returning();
    return result;
  }

  async getAcePledgesByPolitician(politicianId: string): Promise<AcePledgeRequest[]> {
    return await db.select().from(acePledgeRequests)
      .where(eq(acePledgeRequests.politicianId, politicianId))
      .orderBy(desc(acePledgeRequests.createdAt));
  }

  async getAllAcePledgeRequests(status?: string): Promise<(AcePledgeRequest & { sigName?: string; politicianName?: string })[]> {
    const rows = await db
      .select({
        pledge: acePledgeRequests,
        sigName: specialInterestGroups.name,
        politicianName: politicianProfiles.fullName,
      })
      .from(acePledgeRequests)
      .leftJoin(specialInterestGroups, eq(acePledgeRequests.sigId, specialInterestGroups.id))
      .leftJoin(politicianProfiles, eq(acePledgeRequests.politicianId, politicianProfiles.id))
      .where(status ? eq(acePledgeRequests.status, status) : undefined)
      .orderBy(desc(acePledgeRequests.createdAt));
    return rows.map(r => ({ ...r.pledge, sigName: r.sigName ?? undefined, politicianName: r.politicianName ?? undefined }));
  }

  async reviewAcePledgeRequest(id: string, status: string, reviewedBy: number, reviewNote?: string): Promise<AcePledgeRequest> {
    const [result] = await db.update(acePledgeRequests)
      .set({ status, reviewedBy, reviewNote: reviewNote ?? null, updatedAt: new Date() })
  ...
  async createApiKey(data: { userId: string; name: string; keyHash: string; keyPrefix: string }): Promise<ApiKey> {
  ...
  async countActiveApiKeys(userId: string): Promise<number> {
    const rows = await db.select({ cnt: count() }).from(apiKeys)
      .where(and(eq(apiKeys.userId, userId), sql`${apiKeys.revokedAt} IS NULL`));
    return rows[0]?.cnt ?? 0;
  }
  ...
  async createAgentApiKey(data: InsertAgentApiKey): Promise<AgentApiKey> {
    const [key] = await db.insert(agentApiKeys).values(data).returning();
    return key;
  }

  async listAgentApiKeys(): Promise<AgentApiKey[]> {
    return db.select().from(agentApiKeys).orderBy(desc(agentApiKeys.createdAt));
  }

  async getAgentApiKeyById(id: string): Promise<AgentApiKey | undefined> {
    const [key] = await db.select().from(agentApiKeys).where(eq(agentApiKeys.id, id));
    return key;
  }

  async findAgentApiKeyByHash(keyHash: string): Promise<AgentApiKey | undefined> {
    const [key] = await db.select().from(agentApiKeys)
      .where(and(eq(agentApiKeys.keyHash, keyHash), eq(agentApiKeys.status, "active"), sql`${agentApiKeys.revokedAt} IS NULL`));
    return key;
  }

  async updateAgentApiKey(id: string, data: Partial<InsertAgentApiKey>): Promise<AgentApiKey> {
    const [key] = await db.update(agentApiKeys).set(data).where(eq(agentApiKeys.id, id)).returning();
    return key;
  }

  async revokeAgentApiKey(id: string): Promise<void> {
    await db.update(agentApiKeys).set({ status: "revoked", revokedAt: new Date() }).where(eq(agentApiKeys.id, id));
  }
  ...
  async submitPledgeRequest(userId: string, pledgeId: string, statement: string): Promise<any> {
    const [existing] = await db.select().from(pledgeRequests).where(and(eq(pledgeRequests.userId, userId), eq(pledgeRequests.pledgeId, pledgeId))).limit(1);
    if (existing) {
      if (existing.status === "rejected") {
        const [updated] = await db.update(pledgeRequests).set({ statement, status: "pending", reviewNote: null, updatedAt: new Date() }).where(eq(pledgeRequests.id, existing.id)).returning();
        return updated;
      }
      return existing;
    }
    const [created] = await db.insert(pledgeRequests).values({ userId, pledgeId, statement }).returning();
    return created;
  }

  async getUserPledges(userId: string): Promise<any[]> {
    return db.select().from(pledgeRequests).where(eq(pledgeRequests.userId, userId)).orderBy(desc(pledgeRequests.createdAt));
  }

  async getApprovedPledges(userId: string): Promise<any[]> {
    return db.select().from(pledgeRequests).where(and(eq(pledgeRequests.userId, userId), eq(pledgeRequests.status, "approved"))).orderBy(desc(pledgeRequests.createdAt));
  }

  async getPledgeRequests(status?: string): Promise<any[]> {
    const rows = await db
      .select({
        id: pledgeRequests.id,
        userId: pledgeRequests.userId,
        pledgeId: pledgeRequests.pledgeId,
        statement: pledgeRequests.statement,
        status: pledgeRequests.status,
        reviewedBy: pledgeRequests.reviewedBy,
        reviewNote: pledgeRequests.reviewNote,
        createdAt: pledgeRequests.createdAt,
        updatedAt: pledgeRequests.updatedAt,
        username: users.username,
        userAvatar: users.avatar,
        userRole: users.role,
      })
      .from(pledgeRequests)
      .leftJoin(users, eq(pledgeRequests.userId, users.id))
      .where(status ? eq(pledgeRequests.status, status) : undefined)
      .orderBy(desc(pledgeRequests.createdAt));
    return rows;
  }

  async reviewPledgeRequest(id: string, status: string, reviewedBy: string, reviewNote: string): Promise<void> {
    await db.update(pledgeRequests).set({ status, reviewedBy, reviewNote, updatedAt: new Date() }).where(eq(pledgeRequests.id, id));
  }

  async getIssueResponses(userId: string): Promise<IssueResponse[]> {
    return db.select().from(issueResponses).where(eq(issueResponses.userId, userId));
  }

  async upsertIssueResponses(userId: string, responses: Array<{ issueId: string; response: number | null; priority: boolean }>): Promise<void> {
    if (responses.length === 0) return;
    for (const r of responses) {
      await db
        .insert(issueResponses)
        .values({ userId, issueId: r.issueId, response: r.response ?? null, priority: r.priority, updatedAt: new Date() })
        .onConflictDoUpdate({
          target: [issueResponses.userId, issueResponses.issueId],
  ...
  async queueZipCandidateImports(
    zipCode: string,
    candidates: Array<{ name: string; office: string; raceLevel: string; party?: string; city?: string; state?: string; district?: string; matchStatus?: "in_db" | "possible_duplicate" | "new" }>,
    source: string = "background"
  ): Promise<{ queued: number; duplicates: number; total: number }> {
    let queued = 0;
    let duplicates = 0;
    ...
    return { queued, duplicates, total };
  }
  ...
  async previewZipCandidates(
    zipCode: string,
    candidates: Array<{ name: string; office: string; raceLevel: string; party?: string; city?: string; state?: string; district?: string }>
  ): Promise<Array<{ name: string; office: string; raceLevel: string; party?: string; city?: string; state?: string; district?: string; matchStatus: "in_db" | "possible_duplicate" | "new"; matchedProfile?: string }>> {
    const results = [];
    for (const c of candidates) {
      const candidateName = c.name?.trim();
      if (!candidateName) continue;
